...
workflow ONT_LQ {
    ...
    ch versions = ch_versions.mix(CANU_ONT.out.versions)

    FLYE(
        CANU_ONT.out.corrected_trimmed_reads,
        "--nano-raw"
    )
    ch versions = ch_versions.mix(FLYE.out.versions)
    ...
    ch versions = ch_versions.mix(MINIMAP2_ALIGN.out.versions)
    ...
    ch versions = ch_versions.mix(RACON.out.versions)
