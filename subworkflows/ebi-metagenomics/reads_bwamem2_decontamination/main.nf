...
workflow READS_BWAMEM2_DECONTAMINATION {
    ...
    BWAMEM2_MEM_DECONT(ch_reads, ch_reference)
    ch_versions = ch_versions.mix(BWAMEM2_MEM_DECONT.out.versions.first())

    SAMTOOLS_BAM2FQ( BWAMEM2_MEM_DECONT.out.bam.map { meta, bam, _ -> [ meta, bam, meta.single_end == false ] } )
