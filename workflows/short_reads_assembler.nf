...
workflow SHORT_READS_ASSEMBLER {
    ...
    take:
    reads_ch // tuple(meta), path(reads)
    ...
    */
    def reads_by_assembler = reads_ch.map { meta, reads ->
    ...
    extended_reads_qc.branch { meta, _reads, reads_qc_meta ->
    ...
    MEGAHIT(
        qc_filtered_reads.megahit.map { meta, reads, _reads_qc_meta -> [meta, reads] }
    )
    ...
    def assembly = SPADES.out.contigs.mix( megahit_contigs.assembled )
